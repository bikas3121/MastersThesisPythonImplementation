...
with open('Data/data1/ds_6.txt') as f:
    lines = f.readlines()
...
#   pattern_list.append(items)
    PP.setObjective(pv.prod(obj),GRB.MINIMIZE)
    PP.optimize()
    colmn = []
    for v in PP.getVars():
        colmn.append(v.X)
...
len_item = [0]
len_items = []
len_items.append(length[0])
for i in range(1,len(length)):
    c = len_items[i-1] + length[i]
    len_items.append(c)
...
MM = Model('Maximization Problem')

y = MM.addVars(item_count_original, vtype = GRB.INTEGER, name = 'y', obj = value)

x = MM.addVars(len_patt, vtype = GRB.CONTINUOUS, name = 'x')

MM.update()

for i in range(item_count_original):
    MM.addConstr(sum(number_of_items_per_pattern[i][j] * x[j] for j in range(len_patt)) >= demand[i]*y[i])

MM.addConstr(sum(x[i] for i in range(len_patt)) == size[0])

for i in range(item_count_original):
    MM.addConstr(y[i] >= minv[i])
    MM.addConstr(y[i] <= maxv[i])
#M.addConstr(x[2] <= 0)
...
#    b = [c*d for c,d in zip(used_pattern_list[i],height)]
#    ab.append(b)
#    